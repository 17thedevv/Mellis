    ...
    for (auto& block : func.blocks) {
        ...
        for (size_t i = 0; i < block->instructions.size(); ++i) {
            ...
            auto addUse = [&](const mvir::Operand& op) {
                if (auto* loc = mvir::getLocalIf(op)) usesDefs.insert(loc->name);
            };
            ...
            for (const auto& var : usesDefs) {
                ...
                if (tIt != varTypes.end() && tIt->second && tIt->second->needsDrop()) {
                    ...
                    mvir::LocalId dropId; dropId.name = var;
                    newInsts.push_back(std::make_unique<mvir::DropInst>(mvir::Operand(mvir::Place(dropId)), tIt->second));
                    changed = true;
                    }
                }
            }
        ...
        auto addUseTerm = [&](const mvir::Operand& op) {
            if (auto* loc = mvir::getLocalIf(op)) termUses.insert(loc->name);
        };
