...
//
// Semantic Type System for fdlang (Phase 4).
...
#include "fdlang/AST/TypeNode.h" // For BuiltinKind
...
public:
    ...
    FunctionType(std::vector<std::string> names, std::vector<const Type*> params, const Type* ret, bool callSite = false)
        : paramNames(std::move(names)), paramTypes(std::move(params)), returnType(ret), isCallSite(callSite) {}
...
public:
    ...
    const FunctionType* getFunctionType(std::vector<std::string> paramNames, std::vector<const Type*> paramTypes, const Type* returnType, bool isCallSite = false) {
        // Full deduplication skipped for simplicity, just create
        return create<FunctionType>(std::move(paramNames), std::move(paramTypes), returnType, isCallSite);
    }
    const FunctionType* getFunctionType(std::vector<const Type*> paramTypes, const Type* returnType, bool isCallSite = false) {
        std::vector<std::string> emptyNames(paramTypes.size(), "");
        return create<FunctionType>(std::move(emptyNames), std::move(paramTypes), returnType, isCallSite);
    }
