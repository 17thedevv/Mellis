...
use mellis_semantic::{SemanticContext, Resolver, ScopeId};
...
impl ModuleLoader {
    ...
    fn resolve_nested_imports(
        imported_items: &[mellis_ast::Item],
        module_source: &str,
        host_ctx: &mut SemanticContext,
        host_source_map: &mut Vec<(String, String)>,
        ...
        search_paths: &[PathBuf],
        registry: &mut ModuleRegistry,
    ) -> mellis_semantic::ModuleNamespaceMap {
        use mellis_ast::{Decl, ImportKind};

        let mut namespace_map = mellis_semantic::ModuleNamespaceMap::new();
    ...
    fn load_source(
        ...
        let (mut module_arena, module_root_items, _source) = mellis_mlib::MlibReader::read_ast_interface(&mut cursor)
        ...
        let new_file_id = mellis_common::ids::FileId(host_arena.exprs.len() as u32 + 1000); // Placeholder unique FileId
        ...
        let module_scope = host_ctx.symbol_table.create_scope(mellis_semantic::symbol::ScopeKind::Module, Some(mellis_semantic::ScopeId(0)));
        
        let mut imported_items = Vec::new();
        for item in module_root_items {
            if let mellis_ast::Item::Decl(d_id) = item {
                imported_items.push(mellis_ast::Item::Decl(mellis_ast::DeclId(d_id.0 + decl_offset)));
            }
        }
        ...
        if nested_ns.scopes.is_empty() {
            let mut resolver = mellis_semantic::resolver::Resolver::new(
                host_ctx,
                host_arena,
                &_source,
            );
            ...
            let mut resolver = mellis_semantic::resolver::Resolver::new_with_modules(
                host_ctx,
                host_arena,
                &_source,
                &nested_ns,
        ...
        let mut typechecker = mellis_semantic::typechecker::TypeChecker::new(
            host_ctx,
            host_arena,
            &_source,
        );
    ...
    fn load_mlib(
        ...
        let (mut module_arena, module_root_items, _source) = mellis_mlib::MlibReader::read_ast_interface(&mut file)
        ...
        // Generate a new FileId for this loaded module.
        let new_file_id = mellis_common::ids::FileId(host_arena.exprs.len() as u32 + 1000);
