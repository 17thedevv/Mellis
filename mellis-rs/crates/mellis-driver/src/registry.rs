...
use mellis_semantic::symbol::{ProviderId, ScopeKind, Symbol, SymbolKind};
...
pub struct ExternalTraitImplEntry {
    ...
    pub self_type: mellis_semantic::ty::SemanticTypeId,
...
pub struct ProviderInterface {
    ...
    pub lang_items:
        std::collections::HashMap<mellis_semantic::lang_item::LangItem, CanonicalSymbolId>,
    ...
    pub impl_associated_types:
        HashMap<(ExternalImplKey, CanonicalSymbolId), mellis_semantic::ty::SemanticTypeId>,
    pub impl_self_types: HashMap<ExternalImplKey, mellis_semantic::ty::SemanticTypeId>,
...
impl ModuleRegistry {
    fn get_canonical(
        ...
        current_provider_id: mellis_semantic::symbol::ProviderId,
    ) -> CanonicalSymbolId {
    ...
    fn to_external_impl_key(
        key: &mellis_semantic::semantic_tables::ImplKey,
        ctx: &mellis_semantic::SemanticContext,
        current_provider_id: mellis_semantic::symbol::ProviderId,
    ) -> ExternalImplKey {
        ExternalImplKey {
    ...
    pub fn inject_into_ctx(&self, ctx: &mut mellis_semantic::SemanticContext) {
        ...
        for (&provider_id, interface) in &self.interfaces {
            ...
            for (name, root) in &interface.exported_symbols {
                if let Some(existing) = global_roots.get_mut(name) {
                    Self::merge_symbols(existing, root);
                } else {
        ...
        let mut provider_symbol_maps: HashMap<
            ProviderId,
            HashMap<mellis_common::ids::SymbolId, mellis_common::ids::SymbolId>,
        > = HashMap::new();
        ...
        if let Some(cid) = core_id {
            if let Some(core_interface) = self.interfaces.get(&cid) {
                let core_scope = ctx.symbol_table.create_scope(
                    mellis_semantic::symbol::ScopeKind::Module,
                    Some(global_scope),
                );
                ...
                ctx.external_module_scopes
                    .insert("core".to_string(), core_scope);
            }
        }
        ...
        for interface in self.interfaces.values() {
            for ext_sym in &interface.impl_method_symbols {
                let dummy_scope = ctx.symbol_table.create_scope(
                    mellis_semantic::symbol::ScopeKind::Struct,
                    Some(global_scope),
                );
        ...
        for sym_id in 0..ctx.symbol_table.symbols.len() {
            ...
            let canonical = CanonicalSymbolId {
                provider_id: sym
                    .provider_id
                    .unwrap_or(mellis_semantic::symbol::ProviderId(0)),
                decl_id: sym.decl_id,
                name: sym.name.clone(),
            };
        ...
        let resolve_canonical =
            |canonical: &CanonicalSymbolId| -> Option<mellis_common::ids::SymbolId> {
        ...
        for interface in self.interfaces.values() {
            ...
            for entry in &interface.trait_impl_entries {
                if let Some(trait_sym) = resolve_canonical(&entry.trait_id) {
                    ...
                    let dummy_span = mellis_common::Span::default();
                    if ctx
                    ...
                    ctx.tables
                        .trait_impl_entries
                        .push(mellis_semantic::semantic_tables::TraitImplEntry {
    ...
    fn extract_scope(
        ...
        symbol_types: &mut HashMap<
            mellis_common::ids::SymbolId,
            mellis_semantic::ty::SemanticTypeId,
        >,
    ) {
