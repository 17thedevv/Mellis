...
fn run_compiler_with_options(name: &str, src: &str, options: CompilerOptions) -> (bool, Vec<mellis_common::Diagnostic>) {
    let temp = create_temp_dir(name);
    let main_path = temp.join("main.ms");
    fs::write(&main_path, src).unwrap();
...
fn test_case_20_comptime_resource_leak() {
    ...
    func.values.push(ValueData {
        ty: mellis_semantic::SemanticTypeId(0),
    ...
    let leak_check = if !interp.heap.allocations.is_empty() {
        Err(mellis_semantic::ComptimeError::ResourceLeak("unfreed heap allocation".to_string()))
    } else {
        Ok(())
    };
