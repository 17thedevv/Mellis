...
use mellis_driver::{check, check_semantic_only, CompilerOptions};
...
fn test_adversarial_negative_corpus() {
    let corpus: Vec<(&str, &str, &str)> = vec![
        ...
        ("copy_drop_conflict", "import <core>; struct S { x: i32 } impl core::Copy for S {} impl core::Drop for S { fn drop(self: &rw Self) {} } fn main() {}", "E_COPY_DROP_CONFLICT"),
    ];
