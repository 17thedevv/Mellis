...
fn test_semantic_parity() {
    ...
    let provider_path = parity_dir.join("provider.ln");
    let consumer_path = parity_dir.join("consumer.ln");
    ...
    let consumer_src = fs::read_to_string(&consumer_path).expect("failed to read consumer.ln");
    ...
    let provider_src = fs::read_to_string(&provider_path).expect("failed to read provider.ln");
