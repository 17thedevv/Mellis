...
use std::sync::Arc;
use mellis_ast::{AstArena, Decl, DeclId, Expr, Item};
...
impl<'a> AttributeProcessor<'a> {
    ...
    fn validate_compiler_attr(&mut self, decl: &Decl, annot: &mellis_ast::Annotation) {
        let attr_name = self.get_span_text(annot.name);
    ...
    fn handle_repr(&mut self, decl: &Decl, annot: &mellis_ast::Annotation) {
        match decl {
            Decl::Struct { .. } | Decl::Enum { .. } => {}
            _ => {
                self.diagnostics.push(
    ...
    fn handle_test(&mut self, decl: &Decl, annot: &mellis_ast::Annotation) {
        match decl {
            Decl::Function { .. } => {}
            _ => {
                self.diagnostics.push(
    ...
    fn get_expr_identifier_text(&self, expr_id: mellis_ast::ExprId) -> String {
