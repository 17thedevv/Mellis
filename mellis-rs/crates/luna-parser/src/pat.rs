use crate::Parser;
use luna_ast::{PatId, Pattern, StructPatternField};
...
impl<'a> Parser<'a> {
    pub fn parse_pattern(&mut self) -> Result<PatId, ()> {
        ...
        if self.check(TokenKind::Identifier) {
            ...
            // Wait, does luna_lexer have `TokenKind::Underscore`?
            ...
            if segments.len() > 1 || self.check(TokenKind::LParen) || self.check(TokenKind::LBrace)
            {
                if self.match_token(TokenKind::LBrace) {
                    ...
                    if !self.check(TokenKind::RBrace) {
                        loop {
                            ...
                            let pattern = if self.match_token(TokenKind::Colon) {
                                ...
                                // Shorthand
                                let id_pat = self.arena.alloc_pat(Pattern::Identifier {
                                    segments: vec![field_name.span],
                                });
                                Some(id_pat)
                            };
                    ...
                    return Ok(self.arena.alloc_pat(Pattern::Struct {
                        path: segments,
                        fields,
                        has_rest,
                    }));
                    ...
                    return Ok(self.arena.alloc_pat(Pattern::Enum {
                        path: segments,
                        fields,
                    }));
                }
        ...
        self.error_at_current(
            "Expected a pattern (e.g., literal, identifier, or '_') in match arm",
            span,
        );
        Err(())
    }
