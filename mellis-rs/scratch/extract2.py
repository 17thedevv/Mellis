...
def extract_typechecker():
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                # Stop if we hit the multi_replace_file_content tool call that corrupted it
                if 'tool_calls' in data:
                    for tc in data['tool_calls']:
                        if tc.get('function', {}).get('name') == 'default_api:multi_replace_file_content':
                            args = json.loads(tc.get('function', {}).get('arguments', '{}'))
                            if 'typechecker.rs' in args.get('TargetFile', ''):
                                print("Stopping at corrupted multi_replace_file_content call")
                                return write_file(lines_map)
    ...
    write_file(lines_map)

def write_file(lines_map):
    with open('d:/fdlang/mellis-rs/recovered_tc2.rs', 'w', encoding='utf-8') as f:
