...
// MISSING LINE 124
            ...
            (SemanticType::Reference(l1, m1, i1), SemanticType::Reference(l2, m2, i2))
                if m1 == m2 && l1 == l2 =>
            {
                self.unify(i1, i2)
            }
            ...
            (SemanticType::Slice(i1), SemanticType::Slice(i2)) => self.unify(i1, i2),
            (SemanticType::Closure(_, p1, r1), SemanticType::Closure(_, p2, r2))
                if p1.len() == p2.len() =>
            {
            ...
            (SemanticType::Future(o1), SemanticType::Future(o2)) => self.unify(o1, o2),
                    ...
                    Decl::Struct { fields, .. } => {
                        ...
                        if let Some(sym_id) = sym_id_opt {
                            let struct_ty = self
                                .ctx
                                .types
                                .intern(SemanticType::Struct(sym_id, field_tys));
                    ...
                    Decl::Enum { variants, .. } => {
                        ...
                        for variant in variants {
                            ...
                            for field_id in &variant.fields {
                                if let Decl::Param { ty, .. } =
                                    &self.arena.decls[field_id.0 as usize]
                                {
                                    if let Some(t) = ty {
                            ...
                            if field_tys.is_empty() {
                                ...
                                variant_tys
                                    .push(self.ctx.types.intern(SemanticType::Tuple(field_tys)));
                            }
                        }
                        if let Some(sym_id) = sym_id_opt {
                            let enum_ty = self
                                .ctx
                                .types
                                .intern(SemanticType::Enum(sym_id, variant_tys));
            ...
            if let Item::Decl(decl_id) = item {
                ...
                match decl {
                    Decl::Function {
                        ...
                        let func_ty = self.ctx.types.intern(SemanticType::Function {
                            params: param_tys,
                            return_type: actual_ret_ty,
                        });
                        if let Some(sym_id) = sym_id_opt {
                            ...
                            self.ctx
                                ...
                                .function_effects
                                .entry(sym_id)
                                .or_insert_with(crate::effect::EffectSet::pure);
                        }
                    }
                    Decl::Var { type_annot, .. } => {
                        let sym_id_opt = self.ctx.tables.decl_symbols.get(decl_id).copied();
                    ...
                    Decl::Trait { methods, .. } => {
                        for method_id in methods {
                            ...
                            {
                                let sym_id_opt =
                                    self.ctx.tables.decl_symbols.get(method_id).copied();
                                let mut param_tys = Vec::new();
                                for param_id in params {
                                    if let Decl::Param { ty, .. } =
                                        &self.arena.decls[param_id.0 as usize]
                                    {
                                        ...
                                        if let Some(param_sym) =
                                            self.ctx.tables.decl_symbols.get(param_id)
                                        {
                                            self.ctx
                                                .tables
                                                .symbol_types
                                                .insert(*param_sym, param_ty);
                                        }
                                    }
                                }
                                let ret_ty = if let Some(r) = return_type {
        ...
        for item in items {
            if let Item::Decl(decl_id) = item {
                ...
                } = decl
                {
                    ...
                    let self_sym_opt = self_name_opt
                        .and_then(|name| self.ctx.symbol_table.lookup(name, crate::ScopeId(0)));
                    ...
                    if let Some(trait_ty_id) = trait_type {
                        ...
                        if let mellis_ast::Type::Named { segments, .. } = trait_ast_ty {
                            if let Some(last_seg) = segments.last() {
                                let trait_name =
                                    &self.source[last_seg.start as usize..last_seg.end as usize];
                                if trait_name == "Drop" {
                                    if let Some(sym_id) = self_sym_opt {
                                        if let Some(&drop_meth_decl) = methods.first() {
                                            if let Some(&drop_meth_sym) =
                                                self.ctx.tables.decl_symbols.get(&drop_meth_decl)
                                            {
                                                self.ctx
                                                    .tables
                                                    .drop_impls
                                                    .insert(sym_id, drop_meth_sym);
                                            }
                                        }
                                    }
                                } else if let Some(trait_sym) =
                                    self.ctx.symbol_table.lookup(trait_name, crate::ScopeId(0))
                                {
    ...
    pub fn check_object_safety(
        &mut self,
        trait_sym: mellis_common::ids::SymbolId,
        span: Option<mellis_common::Span>,
    ) -> bool {
    ...
    pub fn try_coerce_dyn(
        &mut self,
        expr_id: mellis_ast::ExprId,
        ...
        let (from_inner, to_trait) = match (&from, &to) {
            ...
            (_, SemanticType::DynTrait(trait_sym)) => (from_ty, *trait_sym),
            _ => return false,
        };
        ...
        if self.ctx.tables.trait_impls.contains_key(&key) {
            self.ctx
                .tables
                .dyn_coercions
                .insert(expr_id, (to_trait, concrete_sym));
            return true;
        }
        false
    }
    ...
    pub fn lower_type(&mut self, ast_ty_id: mellis_ast::TypeId) -> SemanticTypeId {
        ...
        let sem_ty_id = match ast_ty {
            ...
            Type::Named {
                ...
                if let Some(symbol) =
                    name.and_then(|name| self.ctx.symbol_table.lookup(name, crate::ScopeId(0)))
                {
                    self.ctx
                        .tables
                        .symbol_types
                        .get(&symbol)
                        .copied()
                        .unwrap_or_else(|| self.ctx.types.new_inference_var())
                } else {
                    self.ctx.types.new_inference_var()
                }
            }
            ...
            Type::Array { element_type, size } => {
                ...
                let resolved_size = match self.eval_comptime_expr(*size) {
                    ...
                    Err(e) => {
                        self.ctx.diagnostics.push(Diagnostic::error(format!(
                            "cannot evaluate array size in comptime: {}",
                            e
                        )));
                        0
                    }
                ...
                self.ctx
                    .types
                    .intern(SemanticType::Array(inner_ty, resolved_size))
            }
            Type::Reference {
                is_mutable, inner, ..
            } => {
                ...
                let mutability = if *is_mutable {
                    crate::ty::Mutability::Mutable
                } else {
                    crate::ty::Mutability::Immutable
                };
                self.ctx.types.intern(SemanticType::Reference(
                    crate::ty::LifetimeId(0),
                    mutability,
                    inner_ty,
                ))
            }
            Type::Pointer {
                ...
                self.ctx
                    .types
                    .intern(SemanticType::Pointer(mutability, inner_ty))
            }
            Type::TraitObject { trait_type } => {
                ...
                if let Type::Named { segments, .. } = trait_ast {
                    ...
                    if let Some(trait_sym) =
                        name.and_then(|n| self.ctx.symbol_table.lookup(n, crate::ScopeId(0)))
                    {
                        ...
                        self.ctx
                            .diagnostics
                            .push(Diagnostic::error("Trait not found for dyn Trait"));
            ...
            Type::MacroCall { span, .. } => {
                self.ctx.diagnostics.push(
                    Diagnostic::error(
                        "internal compiler error: unexpanded macro call in type position",
                    )
                    .with_span(*span),
                );
                self.ctx.types.intern(SemanticType::Error)
            }
            Type::Typeof { expr } => self.typecheck_expr(expr),
            Type::Never => self.ctx.types.intern(SemanticType::Never),
            _ => self.ctx.types.intern(SemanticType::Error),
        };
        self.ctx
            .tables
            .ast_type_to_semantic
            .insert(ast_ty_id, sem_ty_id);
        sem_ty_id
    }
                    ...
                    Decl::Var {
                            ...
                            if let Some(init) = initializer {
                                match self.eval_comptime_expr(*init) {
                                    Ok(val) => {
                                        self.ctx.comptime_values.insert(*init, val.clone());
                                        if let Some(sym_id) =
                                            self.ctx.tables.decl_symbols.get(decl_id).copied()
                                        {
                                            self.ctx.const_values.insert(sym_id, val);
                                        }
                                    }
                                    Err(e) => {
                                        self.ctx.diagnostics.push(
                                            Diagnostic::error(format!(
                                                "cannot evaluate constant in comptime: {}",
                                                e
                                            ))
                                            .with_span(*name),
                                        );
                        ...
                        if let Some(pat_id) = pattern {
                            ...
                            if !self.is_pattern_irrefutable(pat_id, init_ty) {
                                let span = *name;
                                self.ctx.diagnostics.push(
                                    mellis_common::diagnostic::Diagnostic::error("Refutable pattern in local binding. Pattern must be irrefutable (e.g. a struct, tuple, or a single-variant enum).")
                                        .with_span(span)
                                );
                            }
                        }
                        ...
                        if let Some(sym_id) = self.ctx.tables.decl_symbols.get(func).copied() {
                                ...
                                self.ctx.tables.symbol_types.get(&sym_id).copied()
                            {
                                if let Err(e) = self.is_ffi_safe(func_ty) {
                                    self.ctx.diagnostics.push(Diagnostic::error(format!(
                                        "Extern function signature is not FFI-safe: {}",
                                        e
                                    )));
                                }
                            }
        ...
        match pattern {
                        ...
                        } = &decl
                        {
                            for field in fields {
                                if let Some(field_pat) = field.pattern {
                                    let field_name_str = &self.source
                                        [field.name.start as usize..field.name.end as usize];

                                    let mut field_ty = self.ctx.types.new_inference_var();
                                    for struct_field in struct_fields {
                                        let struct_field_name = &self.source[struct_field.name.start
                                            as usize
                                            ..struct_field.name.end as usize];
            ...
            mellis_ast::Pattern::Enum { fields, .. } => {
                ...
                if let Some(variant_sym_id) = self.ctx.tables.pat_symbols.get(pat_id).copied() {
                    ...
                    if let crate::symbol::SymbolKind::EnumVariant(variant_idx) = variant_sym.kind {
                        if let Some(decl_id) = variant_sym.decl_id {
                            if let mellis_ast::Decl::Enum { variants, .. } =
                                &self.arena.decls[decl_id.0 as usize]
                            {
                                if let Some(variant) = variants.get(variant_idx as usize) {
                                    for &param_decl_id in &variant.fields {
                                        if let mellis_ast::Decl::Param {
                                            ty: Some(ty_id), ..
                                        } = &self.arena.decls[param_decl_id.0 as usize]
                                        {
                                            let sem_ty = self
                                                .ctx
                                                .tables
                                            ...
                                            variant_payload_tys
                                                .push(self.ctx.types.new_inference_var());
                                        }
                                    }
                                }
                            }
                ...
                for (i, field) in fields.iter().enumerate() {
                    let field_ty = variant_payload_tys
                        .get(i)
                        .copied()
                        .unwrap_or_else(|| self.ctx.types.new_inference_var());
    ...
    fn is_pattern_irrefutable(&mut self, pat_id: &mellis_ast::PatId, ty: SemanticTypeId) -> bool {
        match &self.arena.pats[pat_id.0 as usize] {
            mellis_ast::Pattern::Wildcard => true,
            mellis_ast::Pattern::Identifier { .. } => {
                if let Some(sym_id) = self.ctx.tables.pat_symbols.get(pat_id) {
                    ...
                    if matches!(sym.kind, crate::symbol::SymbolKind::EnumVariant(_)) {
                        // Check if the enum has 1 variant
                ...
                true // Variable binding is irrefutable
            }
            mellis_ast::Pattern::Literal(_) => false,
            mellis_ast::Pattern::Tuple { elements, .. } => {
                let resolved_ty = self.ctx.types.get(ty).clone();
                if let SemanticType::Tuple(elem_tys) = resolved_ty {
                    elements.iter().enumerate().all(|(i, e)| {
                        if i < elem_tys.len() {
                            self.is_pattern_irrefutable(e, elem_tys[i])
                        } else {
                            true
                        }
                    })
                        ...
                        } = &decl
                        {
                            return fields.iter().all(|field| {
                                if let Some(field_pat) = field.pattern {
                                    ...
                                    self.is_pattern_irrefutable(&field_pat, field_ty)
                                } else {
                                    true
                                }
                            });
    ...
    fn infer_stmt_value_type(&self, stmt_id: &mellis_ast::StmtId) -> Option<SemanticTypeId> {
        match &self.arena.stmts[stmt_id.0 as usize] {
            ...
            Stmt::If {
                ...
                .infer_stmt_value_type(then_branch)
                .or_else(|| self.infer_stmt_value_type(else_branch)),
            _ => None,
        }
    }
        ...
        match stmt {
            ...
            Stmt::If {
                ...
                let bool_ty = self
                    .ctx
                    .types
                    .intern(SemanticType::Primitive(crate::ty::BuiltinType::Bool));
                if self.unify(bool_ty, cond_ty).is_err() {
                    self.ctx
                        .diagnostics
                        .push(Diagnostic::error("if condition must be a boolean"));
                }
            ...
            Stmt::While {
                ...
                if self.unify(bool_ty, cond_ty).is_err() {
                    self.ctx
                        .diagnostics
                        .push(Diagnostic::error("while condition must be a boolean"));
                }
            ...
            Stmt::For {
                ...
                if let Some(c) = cond {
                    ...
                    if self.unify(bool_ty, cond_ty).is_err() {
                        self.ctx
                            .diagnostics
                            .push(Diagnostic::error("for condition must be a boolean"));
                    }
                }
                if let Some(s) = step {
                    self.typecheck_expr(s);
                }

                let mut iter_elem_ty = self.ctx.types.new_inference_var();
                if let Some(iter) = iterable {
                    let iter_ty = self.typecheck_expr(iter);
                    let resolved = self.ctx.types.resolve_inference(iter_ty);
                    if let SemanticType::Range(inner) = self.ctx.types.get(resolved) {
                        iter_elem_ty = *inner;
                    }
                }
                if let Some(pat) = pattern {
                    self.typecheck_pattern(pat, iter_elem_ty);
                }
    ...
    fn typecheck_expr(&mut self, expr_id: &mellis_ast::ExprId) -> SemanticTypeId {
        ...
        let ty_id = match expr {
            Expr::Literal(tok) => {
                ...
                let kind = match tok.kind {
                    ...
                    TokenKind::KwTrue | TokenKind::KwFalse => {
                        SemanticType::Primitive(BuiltinType::Bool)
                    }
                        ...
                        if let crate::SymbolKind::EnumVariant(_) = symbol.kind {
                            if let Some(decl_id) = symbol.decl_id {
                                if let Some(enum_sym_id) =
                                    self.ctx.tables.decl_symbols.get(&decl_id)
                                {
                ...
                if matches!(l_sem_ty, SemanticType::Pointer(_, _)) {
                    ...
                    if matches!(op, BinaryOp::Add | BinaryOp::Sub) {
                        if !self.is_unsafe_context {
                            self.ctx.diagnostics.push(mellis_common::Diagnostic::error(
            ...
            Expr::Call { callee, args, .. } => {
                ...
                if let Expr::Identifier { segments, .. } = callee_expr {
                    ...
                    if name_str == "drop" {
                        ...
                        let diag = if let Some(span) = segments.last() {
                            diag.with_span(*span)
                        } else {
                            diag
                        };
                ...
                let is_struct_method = if let Expr::Member { .. } = callee_expr {
                    self.ctx.tables.expr_symbols.contains_key(callee)
                } else {
                    false
                };
                let is_method_call = (is_dyn_call || is_struct_method)
                    && if let SemanticType::Function { ref params, .. } = callee_ty {
                ...
                if let Some(sym_id) = self.ctx.tables.expr_symbols.get(lvalue).copied() {
                    for lambda_id in &self.active_lambdas {
                        if self
                            ...
                            .expr_captures
                            .get(lambda_id)
                            .map_or(false, |caps| caps.contains(&sym_id))
                        {
                            self.ctx
                                .tables
                                .closure_mutated_captures
                                .entry(*lambda_id)
                                .or_default()
                                .insert(sym_id);
                        }
            ...
            Expr::Member { object, member } => {
                ...
                if let SemanticType::Struct(sym_id, field_tys) = peeled_ty {
                        ...
                        if let Decl::Struct { fields, .. } = &self.arena.decls[decl_id.0 as usize] {
                            if let Some((index, _)) =
                                fields.iter().enumerate().find(|(_, field)| {
                                    &self.source[field.name.start as usize..field.name.end as usize]
                                        == member_name
                                })
                            {
                                self.ctx
                    ...
                    for (impl_key, &impl_decl_id) in &self.ctx.tables.trait_impls {
                        if impl_key.self_type_def == sym_id {
                            if let Decl::Impl { methods, .. } =
                                &self.arena.decls[impl_decl_id.0 as usize]
                            {
                                for &m_id in methods {
                                    if let Decl::Function { name, .. } =
                                        &self.arena.decls[m_id.0 as usize]
                                    {
                                        let m_name =
                                            &self.source[name.start as usize..name.end as usize];
                                        if m_name == member_name {
                                            if let Some(&m_sym) =
                                                self.ctx.tables.decl_symbols.get(&m_id)
                                            {
                                                if let Some(&m_ty) =
                                                    self.ctx.tables.symbol_types.get(&m_sym)
                                                {
                                                    self.ctx
                                                        .tables
                                                        .expr_symbols
                                                        .insert(*expr_id, m_sym);
                                                    self.ctx
                                                        .tables
                                                        .expr_types
                                                        .insert(*expr_id, m_ty);
                    ...
                    self.ctx.diagnostics.push(
                        Diagnostic::error(format!("Unknown field or method '{}'", member_name))
                            .with_span(*member),
                    );
                    let infer = self.ctx.types.new_inference_var();
                    self.ctx.tables.expr_types.insert(*expr_id, infer);
                    ...
                    if let Some(bounds) = self.ctx.tables.trait_bounds.get(&sym_id) {
                        for bound in bounds {
                            if let Some(method_syms) =
                                self.ctx.tables.trait_methods.get(&bound.trait_id)
                            {
                                for (idx, &m_sym) in method_syms.iter().enumerate() {
                                    let sym = self.ctx.symbol_table.get_symbol(m_sym);
                                    if sym.name == member_name {
                ...
                self.ctx.diagnostics.push(
                    Diagnostic::error("Member access requires a struct or trait object")
                        .with_span(*member),
                );
                self.ctx.types.new_inference_var()
            }
            Expr::StructInit { path, fields, .. } => {
                ...
                let Some(symbol) = self.ctx.symbol_table.lookup(name, crate::ScopeId(0)) else {
                    self.ctx.diagnostics.push(
                        Diagnostic::error(format!("Unknown struct '{}'", name))
                            .with_span(*name_span),
                    );
                    return self.ctx.types.new_inference_var();
                };
                let struct_ty = self
                    ...
                    .copied()
                    .unwrap_or_else(|| self.ctx.types.new_inference_var());
                let SemanticType::Struct(_, field_tys) = self.ctx.types.get(struct_ty).clone()
                else {
                    self.ctx.diagnostics.push(
                ...
                for field in fields {
                    let field_name =
                        &self.source[field.name.start as usize..field.name.end as usize];
                    if let Some((index, _)) =
                        declared_fields.iter().enumerate().find(|(_, declared)| {
                            &self.source[declared.name.start as usize..declared.name.end as usize]
                                == field_name
                        })
                    {
            ...
            Expr::ArrayLiteral { elements } => {
                ...
                self.ctx
                    .types
                    .intern(SemanticType::Array(elem_ty, elements.len() as u64))
            }
            ...
            Expr::TupleIndex { object, index } => {
                ...
                match base_ty {
                    ...
                    _ => {
                        self.ctx
                            .diagnostics
                            .push(Diagnostic::error("Cannot index into a non-tuple type"));
                        self.ctx.types.new_inference_var()
                    }
                }
            }
            ...
            Expr::MethodCall {
                ...
                let obj_ty = self.ctx.types.get(obj_ty_id).clone();
                let member_name =
                    &self.source[method_name.start as usize..method_name.end as usize];

                // 1. Check if obj_ty is DynTrait or 
                let dyn_trait_sym = match &obj_ty {
                    SemanticType::DynTrait(sym) => Some(*sym),
                    SemanticType::Pointer(_, inner) | SemanticType::Reference(_, _, inner) => {
                        if let SemanticType::DynTrait(sym) = self.ctx.types.get(*inner) {
                        ...
                        for (idx, &m_sym) in method_syms.iter().enumerate() {
                            ...
                            if sym.name == member_name {
                                ...
                                if let Some(&m_ty) = self.ctx.tables.symbol_types.get(&m_sym) {
                                    ...
                                    {
                                        let expected_params = if params.len() == args.len() + 1 {
                                            &params[1..]
                                        } else {
                                            &params[..]
                                        };
                                        for (i, arg) in args.iter().enumerate() {
                                            ...
                                            if let Some(&expected_p) = expected_params.get(i) {
                                                if !self
                                                    .try_coerce_dyn(arg.value, arg_ty, expected_p)
                                                {
                                                    let _ = self.unify(expected_p, arg_ty);
                                                }
                                            }
                                                    ...
                                                    self.ctx.tables.symbol_types.get(&m_sym)
                                                {
                                                    ...
                                                    } = self.ctx.types.get(m_ty).clone()
                                                    {
                                                        ...
                                                        for (i, arg) in args.iter().enumerate() {
                                                            let arg_ty =
                                                                self.typecheck_expr(&arg.value);
                                                            if let Some(&expected_p) =
                                                                expected_params.get(i)
                                                            {
                                                                if !self.try_coerce_dyn(
                                                                    arg.value, arg_ty, expected_p,
                                                                ) {
                                                                    let _ = self
                                                                        .unify(expected_p, arg_ty);
                                                                }
                                                            }
                                                        }
                                                        return return_type;
            ...
            Expr::Sizeof { target_type } => {
                ...
                self.ctx
                    .types
                    .intern(SemanticType::Primitive(crate::ty::BuiltinType::I32))
            }
                            ...
                            if !covered_bools.contains(&true) || !covered_bools.contains(&false) {
                                self.ctx.diagnostics.push(
                                    mellis_common::diagnostic::Diagnostic::error(
                                        "Match is not exhaustive. Missing boolean values.",
                                    )
            ...
            Expr::Lambda {
                ...
                let mut param_tys = Vec::new();
                for param_id in params {
                    if let mellis_ast::Decl::Param { ty, .. } =
                        &self.arena.decls[param_id.0 as usize]
                    {
                ...
                if let Some(body_ty) = self.infer_stmt_value_type(body) {
                    if let Err(_) = self.unify(ret_ty_id, body_ty) {
                        self.ctx
                            .diagnostics
                            .push(Diagnostic::error("Lambda return type mismatch"));
                    }
                }
                ...
                if let Some(captures) = self.ctx.tables.expr_captures.get(expr_id).cloned() {
                    for (field, sym_id) in captures.into_iter().enumerate() {
                        let sym_ty = self
                            ...
                            .get(&sym_id)
                            .copied()
                            .unwrap_or_else(|| self.ctx.types.intern(SemanticType::Error));
                        let is_mutated = self
                            .ctx
                            .tables
                            .closure_mutated_captures
                            .get(expr_id)
                            .map_or(false, |mutated| mutated.contains(&sym_id));
                        let mode = if *is_move {
                        ...
                        let env_ty = match mode {
                            crate::semantic_tables::CaptureMode::MutableBorrow => {
                                self.ctx.types.intern(SemanticType::Pointer(
                                    crate::ty::Mutability::Mutable,
                                    sym_ty,
                                ))
                            }
                        ...
                        let is_mutable = self
                            ...
                            .symbol_decls
                            .get(&sym_id)
                            .and_then(|decl_id| match &self.arena.decls[decl_id.0 as usize] {
                ...
                self.ctx
                    .tables
                    .closure_env_ptr_types
                    .insert(*expr_id, env_ptr_ty);

                self.ctx
                    .types
                    .intern(SemanticType::Closure(*expr_id, param_tys, ret_ty_id))
            }
            Expr::Try { expr: e, .. } => self.typecheck_expr(e),
            Expr::Await { expr } => {
            ...
            Expr::Unary { op, operand } => {
                ...
                match op {
                    UnaryOp::Ref => self.ctx.types.intern(SemanticType::Pointer(
                        crate::ty::Mutability::Immutable,
                        inner_ty,
                    )),
                    UnaryOp::RefMut => {
                    ...
                    UnaryOp::Deref => match self.ctx.types.get(inner_ty) {
                        SemanticType::Pointer(_, pointee)
                        | SemanticType::Reference(_, _, pointee) => *pointee,
                        _ => inner_ty,
                    },
                    _ => inner_ty,
                }
    ...
    fn emit_type_mismatch(&mut self, msg: &str, expr_id: &mellis_ast::ExprId) {
        let mut diag = mellis_common::diagnostic::Diagnostic::error(msg);
        ...
        if let mellis_ast::Expr::Identifier { segments, .. } = expr {
            if let Some(sym_id) = self.ctx.tables.expr_symbols.get(expr_id) {
                ...
                if matches!(symbol.kind, crate::symbol::SymbolKind::Constant) {
                    let mut diag = mellis_common::diagnostic::Diagnostic::error(
