...
use mellis_ast::{Item, Stmt, StmtId, ForKind};
...
impl<'a> Parser<'a> {
    ...
    pub fn parse_block_stmt(&mut self) -> Result<StmtId, ()> {
        ...
        if let Some(Item::Stmt(last_stmt_id)) = body.last() {
            if let Stmt::Expr { expr, has_semicolon: false } = &self.arena.stmts[last_stmt_id.0 as usize] {
                tail_expr = Some(*expr);
                body.pop();
            }
        }
    ...
    fn parse_expression_stmt(&mut self) -> Result<StmtId, ()> {
        ...
        if self.check(TokenKind::RBrace) {
            return Ok(self.arena.alloc_stmt(Stmt::Expr { expr, has_semicolon: false }));
        }
        self.consume(TokenKind::Semi, "Expected ';' after expression")?;
        Ok(self.arena.alloc_stmt(Stmt::Expr { expr, has_semicolon: true }))
    }
    ...
    fn parse_if_stmt(&mut self) -> Result<StmtId, ()> {
        ...
        Ok(self.arena.alloc_stmt(Stmt::If { condition, then_branch, else_branch }))
    }
    ...
    fn parse_while_stmt(&mut self) -> Result<StmtId, ()> {
        ...
        Ok(self.arena.alloc_stmt(Stmt::While { label: None, condition, body }))
    }
    ...
    fn parse_for_stmt(&mut self) -> Result<StmtId, ()> {
        ...
        if self.match_token(TokenKind::At) {
            ...
            self.error_at_current("Macro for loops are not fully implemented", self.previous().span);
            return Err(());
        }
        ...
        if self.check(TokenKind::KwDec) || self.check(TokenKind::KwConst) || self.check(TokenKind::Semi) {
            kind = ForKind::CStyle;
        ...
        Ok(self.arena.alloc_stmt(Stmt::For {
            kind, label: None, binding_name, pattern, iterable, init, cond, step, body
        }))
    }

    fn parse_return_stmt(&mut self) -> Result<StmtId, ()> {
        let value = if !self.check(TokenKind::Semi) { Some(self.parse_expression(true)?) } else { None };
    ...
    fn parse_break_stmt(&mut self) -> Result<StmtId, ()> {
        let label = if self.check(TokenKind::Lifetime) { Some(self.advance().span) } else { None };
...
with open(r"d:\fdlang\mellis-rs\crates\mellis-parser\src\stmt.rs", "w", encoding="utf-8") as f:
    f.write(rust_code)

print("Created stmt.rs")
